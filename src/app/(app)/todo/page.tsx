  ...
  const fetchData = useCallback(async () => {
    ...
    const [{ data: profile }, { data: todosData }, { data: recordsData }, { data: tagsData }] =
      await Promise.all([
        ...
        supabase.from("todos").select("*, tags(name, color)").eq("user_id", user.id).order("is_important", { ascending: false }).order("created_at", { ascending: true }),
    ...
    (recordsData || []).forEach((r: DailyRecord) => {
      recordMap[r.todo_id] = r;
    });
  ...
  if (loading) {
    return (
      <div className="flex flex-1 items-center justify-center">
        <p className="font-pixel text-sm text-theme-muted">불러오는 중...</p>
      </div>
    );
  }
