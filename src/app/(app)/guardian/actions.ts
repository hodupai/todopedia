...
// ── 가디 페이지 초기 데이터 (단일 RPC: growth + care + activity) ──
export async function getGuardianPageData() {
  ...
  const { data } = await supabase.rpc("get_guardian_page_data", { p_user_id: user.id });
  const growth = data?.growth;

  if (!growth || growth.status === "no_active_guardian") {
    return {
      state: "idle" as const,
      growthData: null,
      careStatus: { food: false, play: false, hygiene: false, sleep: false },
  ...
  const cared = new Set<string>(data?.care || []);
  const careStatus: CareStatus = {
    food: cared.has("food"),
    play: cared.has("play"),
  ...
  const activityLogs: ActivityLog[] = (data?.activity || []).map((a: { type: ActivityLog["type"]; title: string; gold: number; time: string }) => ({
    type: a.type,
    title: a.title,
    gold: a.gold,
    time: a.time,
