...
export type GuardianCollectionInitial = {
  types: GuardianType[];
...
export default function CollectionClient({
  ...
  initialGuardian: GuardianCollectionInitial;
}) {
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4">
      ...
      {tab === "가디언 도감" ? <GuardianCollection initial={initialGuardian} /> : <ItemCollection />}
    </div>
  );
}

// ── 가디언 도감 ──
function GuardianCollection({ initial }: { initial: GuardianCollectionInitial }) {
  const [types] = useState<GuardianType[]>(initial.types);
  const [items] = useState<CollectionItem[]>(initial.items);
