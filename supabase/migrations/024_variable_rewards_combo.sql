-- 투두 완료 보상에 변동성/콤보 추가
-- - 5% 확률 크리티컬: 골드 2.5배 (200 → 500)
-- - 콤보: 직전 완료로부터 5분 이내면 combo_count++, 3 이상에서 +100G 보너스
-- - 콤보 끊김(5분 초과)이면 1로 리셋
-- 결과 JSON에 crit/combo 필드 추가해 클라가 메시지를 표시

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS combo_count INT NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS last_complete_at TIMESTAMPTZ;
...
BEGIN
  ...
  IF v_new_completed THEN
    ...
    IF v_eligible_for_gold THEN
      ...
      -- 콤보 처리: 직전 완료가 5분 이내면 +1, 아니면 1로 리셋
  ...
  -- profiles 갱신: gold + combo + last_complete_at
  IF v_new_completed THEN
    UPDATE profiles
...
BEGIN
  ...
  IF v_is_completed AND v_eligible THEN
    UPDATE profiles
