-- complete_todo / complete_loop의 stale FOUND 버그 수정
-- SELECT * INTO v_record 직후의 FOUND가 이후 SELECT COUNT(*) 등에 의해 덮어써져,
-- 마지막 IF FOUND THEN UPDATE ELSE INSERT 분기가 항상 UPDATE를 타고
-- v_record.id가 NULL이라 0 rows affected → daily_records에 아무것도 안 써지던 버그.
-- 행 존재 여부를 v_record_exists 변수에 캡처해서 사용하도록 변경.
